...
const THREADS_COLLAPSED_PREFIX = "chenchen:workspace:claude-threads-collapsed:v1";

function modelSlotStorageKey(authorId: string) {
  return `${MODEL_SLOT_PREFIX}:${authorId.toLowerCase()}`;
}

function threadsCollapsedStorageKey(authorId: string) {
  return `${THREADS_COLLAPSED_PREFIX}:${authorId.toLowerCase()}`;
}
...
export function WorkspaceClaudeChat({ authorId }: { authorId: string }) {
  ...
  const threadsCollapsedKey = useMemo(
    () => threadsCollapsedStorageKey(authorId),
    [authorId],
  );
  ...
  const fmt = useCallback(
    (keyName: string, vars: Record<string, string | number>) =>
  ...
  useEffect(() => {
    try {
      const raw = window.localStorage.getItem(threadsCollapsedKey);
      setThreadsCollapsed(raw === "1");
    } catch {
  ...
  const sessionIndexById = useMemo(() => {
    ...
    sortedSessions.forEach((s, idx) => map.set(s.id, idx + 1));
    return map;
  }, [sortedSessions]);
  ...
  const toggleThreadsCollapsed = useCallback(() => {
    setThreadsCollapsed((prev) => {
      const next = !prev;
      try {
        window.localStorage.setItem(threadsCollapsedKey, next ? "1" : "0");
      } catch {
        /* ignore */
      }
      return next;
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {sortedSessions.map((s) => (
            <option key={s.id} value={s.id}>
              {fmt("workspace.aiChatThreadLabel", {
                index: sessionIndexById.get(s.id) ?? 0,
              })}
              {" · "}
              {s.title}
            </option>
        ...
        >
          <li className="mb-1 flex items-center justify-between px-2 py-1">
            ...
            <button
              ...
              className="rounded border border-zinc-700 px-1.5 py-0.5 text-[10px] text-zinc-400 hover:border-cyan-500/50 hover:text-cyan-200"
            >
              {threadsCollapsed
                ? t("workspace.aiChatExpandThreads")
                : t("workspace.aiChatCollapseThreads")}
            </button>
          </li>
          {sortedSessions.map((s) => (
            <li key={s.id}>
