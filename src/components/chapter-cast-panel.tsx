...
export function ChapterCastPanel({
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-hidden">
      <div className="shrink-0 space-y-2 px-2 pt-1">
        <button
          type="button"
          disabled={extractDisabled || extractLoading}
          onClick={() => void onExtract()}
          className="w-full rounded-lg border border-violet-400/80 bg-violet-50 px-2 py-2 text-xs font-medium text-violet-950 disabled:cursor-not-allowed disabled:opacity-45 dark:border-violet-600 dark:bg-violet-950/40 dark:text-violet-100"
        >
          {extractLoading ? "AI 正在分析本章正文…" : "AI 抽取本章登场人物 → JSON"}
        </button>
        {versions.length > 0 ? (
          <div>
      ...
      {!loading && versions.length === 0 ? (
        <p className="px-2 text-center text-[11px] leading-snug text-neutral-500">
          尚无抽取结果。点击上方按钮分析当前章正文，每人写入一个 JSON（新版本目录 v1、v2…）。
        </p>
      ) : null}

      {files.length > 0 ? (
        <>
          <div className="shrink-0 border-b border-neutral-200 px-1 dark:border-neutral-700">
            <div
              ...
              {files.map((f) => {
                ...
                return (
                  <button
                    ...
                    className={[
                      "shrink-0 rounded-t-md border px-2.5 py-1.5 text-left text-[11px] font-medium transition-colors",
                      sel
                        ? "border-b-transparent border-neutral-300 bg-white text-violet-800 dark:border-neutral-600 dark:bg-neutral-900 dark:text-violet-200"
                        : "border-transparent bg-transparent text-neutral-600 hover:bg-neutral-100 dark:text-neutral-400 dark:hover:bg-neutral-800/80",
                    ].join(" ")}
                    title={f.fileName}
                  >
                    <span className="block max-w-[5.5rem] truncate">{f.payload.character.name}</span>
                  </button>
                );
              })}
            </div>
